...
#Report
writeLines(paste(date(), "=> Processing", tax.id, subj))
################################################################################
################################################################################
...
###################################
#Skip current subject if it is not represented in both sites of interest
if (!(subj %in% subjects.with_site_1 & subj %in% subjects.with_site_2)) {writeLines(paste(date(), "=> Skip, no paired data:", tax.id, subj)); q("no")}

#Preallocate
n.allele <- nrow(obs.allele)
if (calculate.full) {bg.allele.subject.site_1.full <- bg.allele.subject.site_2.full <- numeric(length=n.allele)}
bg.allele.subject.site_1.cohort <- bg.allele.subject.site_2.cohort <- numeric(length=n.allele)
...
if (subj %in% subjects.with_site_1) {
  ...
  if (calculate.full) {
    ...
    if (length(idx.full) > 10) {
      ...
      bg.allele.subject.site_1.full <- rowSums(inc.allele.subject.site_1[, idx.full]) / rowSums(obs.allele.subject.site_1[, idx.full])
    }
  }
  ...
  if (length(idx.cohort) > 10) {
    ...
    bg.allele.subject.site_1.cohort <- rowSums(inc.allele.subject.site_1[, idx.cohort]) / rowSums(obs.allele.subject.site_1[, idx.cohort])
  }
}
...
if (subj %in% subjects.with_site_2) {
  ...
  if (calculate.full) {
    ...
    if (length(idx.full) > 10) {
      #Calculate allele frequencies against this global background
      bg.allele.subject.site_2.full <- rowSums(inc.allele.subject.site_2[, idx.full]) / rowSums(obs.allele.subject.site_2[, idx.full])
    }
  }
  ...
  if (length(idx.cohort) > 10) {
    #Calculate allele frequencies against cohort-specific background
    bg.allele.subject.site_2.cohort <- rowSums(inc.allele.subject.site_2[, idx.cohort]) / rowSums(obs.allele.subject.site_2[, idx.cohort])
  }
}
...
###################################
#=> p(site_1 & site_2)
bg.log_p.cohort.1_1 <- log10(bg.allele.subject.site_1.cohort * bg.allele.subject.site_2.cohort)
#=> p(site_1 & !site_2)
bg.log_p.cohort.1_0 <- log10(bg.allele.subject.site_1.cohort * (1 - bg.allele.subject.site_2.cohort))
#=> p(!site_1 & site_2)
bg.log_p.cohort.0_1 <- log10((1 -bg.allele.subject.site_1.cohort) * bg.allele.subject.site_2.cohort)
#=> p(!site_1 & !site_2)
bg.log_p.cohort.0_0 <- log10((1 -bg.allele.subject.site_1.cohort) * (1 - bg.allele.subject.site_2.cohort))

#Add for full background if needed
if (calculate.full) {
  bg.log_p.full.1_1 <- log10(bg.allele.subject.site_1.full * bg.allele.subject.site_2.full)
  bg.log_p.full.1_0 <- log10(bg.allele.subject.site_1.full * (1 - bg.allele.subject.site_2.full))
  bg.log_p.full.0_1 <- log10((1 - bg.allele.subject.site_1.full) * bg.allele.subject.site_2.full)
  bg.log_p.full.0_0 <- log10((1 - bg.allele.subject.site_1.full) * (1 - bg.allele.subject.site_2.full))
}
###################################
...
keep.alleles.cohort <- is.finite(S.max_theo.bg.cohort)
#Skip current subject.timepoint if too few alleles with finite background frequencies are left
if (sum(keep.alleles.cohort) < 100) {writeLines(paste(date(), "=> Quit: too few alleles to consider in", tax.id, taxon, site_1, site_2, subj)); q("no")}
obs.allele.cohort <- obs.allele[keep.alleles.cohort, ]
inc.allele.cohort <- inc.allele[keep.alleles.cohort, ]
bg.log_p.cohort.1_1 <- bg.log_p.cohort.1_1[keep.alleles.cohort]
bg.log_p.cohort.0_0 <- bg.log_p.cohort.0_0[keep.alleles.cohort]
bg.log_p.cohort.1_0 <- bg.log_p.cohort.1_0[keep.alleles.cohort]
bg.log_p.cohort.0_1 <- bg.log_p.cohort.0_1[keep.alleles.cohort]
S.max_theo.bg.cohort <- S.max_theo.bg.cohort[keep.alleles.cohort]
####################################
if (calculate.full) {
  keep.alleles.full <- is.finite(S.max_theo.bg.full)
  obs.allele <- obs.allele[keep.alleles.full, ]
  inc.allele <- inc.allele[keep.alleles.full, ]
  bg.log_p.full.1_1 <- bg.log_p.full.1_1[keep.alleles.full]
  bg.log_p.full.0_0 <- bg.log_p.full.0_0[keep.alleles.full]
  bg.log_p.full.1_0 <- bg.log_p.full.1_0[keep.alleles.full]
  bg.log_p.full.0_1 <- bg.log_p.full.0_1[keep.alleles.full]
  S.max_theo.bg.full <- S.max_theo.bg.full[keep.alleles.full]
}
################################################################################
################################################################################
...
curr.s_t.oi <- curr.pairs.subject_timepoint[curr.pairs.subject == subj]
...
get.S <- function(pair, bg="full") {
  #Get indices of alleles to consider
  if (bg == "full") {
    idx.obs <- rowSums(obs.allele[, pair]) == 2
  } else {
    idx.obs <- rowSums(obs.allele.cohort[, pair]) == 2
  }
  ...
  if (bg == "full") {
    ...
    curr.inc.allele <- inc.allele.cohort[, pair]
  }
  ...
  if (bg == "full") {
    ...
    S.max_theo <- sum(S.max_theo.bg.full[idx.obs])
  } else {
    S.agree <- (sum(bg.log_p.cohort.1_1[idx.1_1], na.rm=T) + sum(bg.log_p.cohort.0_0[idx.0_0], na.rm=T))
    S.disagree <- (sum(bg.log_p.cohort.1_0[idx.1_0], na.rm=T) + sum(bg.log_p.cohort.0_1[idx.0_1], na.rm=T))
    S.max_theo <- sum(S.max_theo.bg.cohort[idx.obs])
  }
...
for (s_t in curr.s_t.oi) {
  ...
  #Skip if there's no paired observation for current s_t
  if (!data.timepoint[s_t, paste0("paired.", site_1.short, "_", site_2.short)]) {next()}
  ###################################
  ...
  if (nrow(pairs.cohort) < 2) {writeLines(paste(date(), "=> Quit: too few cohort-specific background pairs in", tax.id, taxon, site_1, site_2, subj)); q("no")}
  ...
  if (calculate.full) {
    idx.pairs.full.bg <- (cp.family[,1] != cp.family[,2]) & cp.family[,1] != fam & cp.family[,2] != fam
    ...
    if (sum(idx.pairs.full.bg) > PARAM$max.background_size) {
      curr.S.full <- t(apply(curr.sample_pairs[sample(which(idx.pairs.full.bg), size=PARAM$max.background_size, replace=F), ], 1, get.S, bg="full"))
    } else {
      curr.S.full <- t(apply(curr.sample_pairs[idx.pairs.full.bg, ], 1, get.S, bg="full"))
    }
  ...
  idx.pairs.cohort.bg <- (pairs.cohort.family[,1] != pairs.cohort.family[,2]) & pairs.cohort.family[,1] != fam & pairs.cohort.family[,2] != fam
  ...
  if (sum(idx.pairs.cohort.bg) > PARAM$max.background_size) {
    curr.S.cohort <- t(apply(pairs.cohort[sample(which(idx.pairs.cohort.bg), size=PARAM$max.background_size, replace=F), ], 1, get.S, bg=cohort.bg))
  } else {
    curr.S.cohort <- t(apply(pairs.cohort[idx.pairs.cohort.bg, ], 1, get.S, bg=cohort.bg))
  }
  ...
  if (calculate.full) {
    ...
    curr.S.full.indiv <- t(apply(curr.sample_pairs[idx.pairs.full.bg.indiv, ], 1, get.S, bg="full"))
  ...
  curr.S.cohort.indiv <- t(apply(pairs.cohort[idx.pairs.cohort.bg.indiv, ], 1, get.S, bg=cohort.bg))
  ...
  ###################################
  collect.S.intra.cohort <- get.S(pairs.cohort[pairs.cohort.st[,1] == s_t & pairs.cohort.st[,2] == s_t, ], bg=cohort.bg)
  names(collect.S.intra.cohort) <- c("S.agree", "S.disagree", "S.max_theo", "S")
  if (calculate.full) {
    collect.S.intra.full <- get.S(curr.sample_pairs[cp.subject.timepoint[,1] == s_t & cp.subject.timepoint[,2] == s_t, ], bg="full")
  ...
  ###################################
  writeLines(paste(date(), "=> Done with subject.timepoint", s_t, "in", tax.id, taxon))
  ###################################
}

###################################
#Save current data
save(data.bg.full, data.bg.full.indiv, data.bg.cohort, data.bg.cohort.indiv, data.s_t.oi, file=paste0(PARAM$folder.results, "detect.transmission/", tax.id, ".", print.taxon,  ".detect_transmission.", site_1.short, "_", site_2.short, ".", subj, ".RData"))
save(S.bg.full, S.bg.full.indiv, S.bg.cohort, S.bg.cohort.indiv, file=paste0(PARAM$folder.results, "detect.transmission/", tax.id, ".", print.taxon,  ".background_scores.", site_1.short, "_", site_2.short, ".", subj, ".RData"))
#save(S.bg.full.Z, S.bg.full.indiv.Z, S.bg.cohort.Z, S.bg.cohort.indiv.Z, file=paste0(PARAM$folder.results, "detect.transmission/", tax.id, ".", print.taxon,  ".background_scores.z_transformed.", site_1.short, "_", site_2.short, ".", subj, ".RData"))
###################################
################################################################################
################################################################################
